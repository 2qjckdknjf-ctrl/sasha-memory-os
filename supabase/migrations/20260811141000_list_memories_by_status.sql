-- Review queue: list memories filtered by status (candidate/disputed/...)
...
AS $$
  SELECT app.api_list_memories(
    p_secret, p_subject_id, p_workspace_id, p_project_id, p_status, p_limit
  );
$$;

GRANT EXECUTE ON FUNCTION app.api_list_memories(text, uuid, uuid, uuid, text, integer)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.api_list_memories(text, uuid, uuid, uuid, text, integer)
  TO anon, authenticated, service_role;
