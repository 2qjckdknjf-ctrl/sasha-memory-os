-- Record OAuth exchange fingerprint/mode without persisting tokens or codes

DROP FUNCTION IF EXISTS public.api_oauth_complete_stub(text, uuid, text, text, text);
DROP FUNCTION IF EXISTS app.api_oauth_complete_stub(text, uuid, text, text, text);
...
BEGIN
  ...
  IF v_mode NOT IN ('stub', 'credentials_ready') THEN
