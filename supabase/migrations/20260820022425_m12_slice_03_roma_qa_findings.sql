...
CREATE OR REPLACE FUNCTION app.api_enqueue_roma_project_findings(
  p_secret text,
  p_subject_id uuid,
  p_workspace_id uuid,
  p_project_id uuid,
...
DECLARE
  ...
  v_reason text := coalesce(
    ...
    'Generate audited ROMA QA findings for one explicit project.'
  );
  v_idem text := format(
    'roma-project-findings/%s/%s',
    p_project_id::text,
    coalesce(
...
BEGIN
  ...
  ELSE
    ...
    VALUES (
      ...
      'project',
      p_project_id,
      'roma.project_findings.requested',
      jsonb_build_object(
        'jobId', v_job_id,
...
CREATE OR REPLACE FUNCTION public.api_enqueue_roma_project_findings(
  p_secret text,
  p_subject_id uuid,
  p_workspace_id uuid,
  p_project_id uuid,
...
SET search_path = public, app, extensions
AS $$
  SELECT app.api_enqueue_roma_project_findings(
    p_secret,
    p_subject_id,
    p_workspace_id,
    p_project_id,
...
GRANT EXECUTE ON FUNCTION app.api_enqueue_roma_project_findings(text, uuid, uuid, uuid, text, text)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.api_enqueue_roma_project_findings(text, uuid, uuid, uuid, text, text)
  TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION app.api_claim_roma_project_findings_jobs(
  p_secret text,
  p_subject_id uuid,
  p_workspace_id uuid,
...
BEGIN
  ...
  WITH eligible AS (
    ...
    WHERE j.workspace_id = p_workspace_id
      AND j.job_type = 'roma_project_findings'
      AND j.status = 'queued'
...
CREATE OR REPLACE FUNCTION public.api_claim_roma_project_findings_jobs(
  p_secret text,
  p_subject_id uuid,
  p_workspace_id uuid,
  p_limit integer DEFAULT 10,
...
SET search_path = public, app, extensions
AS $$
  SELECT app.api_claim_roma_project_findings_jobs(
    p_secret,
    p_subject_id,
    p_workspace_id,
    p_limit,
...
GRANT EXECUTE ON FUNCTION app.api_claim_roma_project_findings_jobs(text, uuid, uuid, integer, uuid)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.api_claim_roma_project_findings_jobs(text, uuid, uuid, integer, uuid)
  TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION app.api_retry_roma_project_findings(
  p_secret text,
  p_subject_id uuid,
  p_job_id uuid,
...
DECLARE
  ...
  v_error text := coalesce(
    ...
    'retryable ROMA project findings failure'
  );
BEGIN
  PERFORM app.assert_api_secret(p_secret);
  PERFORM app.with_subject(p_subject_id);
...
CREATE OR REPLACE FUNCTION public.api_retry_roma_project_findings(
  p_secret text,
  p_subject_id uuid,
  p_job_id uuid,
  p_error text DEFAULT NULL
...
SET search_path = public, app, extensions
AS $$
  SELECT app.api_retry_roma_project_findings(
    p_secret,
    p_subject_id,
    p_job_id,
    p_error
...
GRANT EXECUTE ON FUNCTION app.api_retry_roma_project_findings(text, uuid, uuid, text)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.api_retry_roma_project_findings(text, uuid, uuid, text)
  TO anon, authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION app.api_complete_roma_project_findings(text, uuid, uuid, text, text, uuid, uuid, integer)
  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.api_complete_roma_project_findings(text, uuid, uuid, text, text, uuid, uuid, integer)
  TO anon, authenticated, service_role;
