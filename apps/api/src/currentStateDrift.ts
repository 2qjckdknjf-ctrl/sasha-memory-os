...
export function validateCurrentStateDrift(
  ...
  if (manifest.currentMilestone !== 'M17.1-entity-graph-foundation') {
    findings.push({
      code: 'current-milestone',
      message: `expected currentMilestone M17.1-entity-graph-foundation, got ${manifest.currentMilestone}`,
    });
  }

  if (manifest.completedThrough !== 'M17.1') {
    findings.push({
      code: 'completed-through',
      message: `expected completedThrough M17.1 after entity graph foundation pack, got ${manifest.completedThrough}`,
    });
  }

  if (manifest.nextSlice !== 'M17.2-entity-resolution') {
    findings.push({
      code: 'next-slice',
      message: `expected nextSlice M17.2-entity-resolution, got ${manifest.nextSlice}`,
    });
  }
