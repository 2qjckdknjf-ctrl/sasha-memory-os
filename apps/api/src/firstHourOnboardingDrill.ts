...
export function evaluateFirstHourOnboardingReport(
  ...
  if (!report.blockedFallbackProjectIds.includes(AISTROYKA_PROJECT_ID)) {
    errors.push(
      `first-hour onboarding manifest must block AISTROYKA fallback ${AISTROYKA_PROJECT_ID}`,
    );
  }
...
export function resolveFirstHourOnboardingDrillConfig(
  ...
  if (projectId === AISTROYKA_PROJECT_ID) {
    throw new Error(
      `AISTROYKA fallback project_id ${AISTROYKA_PROJECT_ID} is not allowed for the bounded first-hour onboarding drill`,
    );
  }
  return {
    fixtureDir,
