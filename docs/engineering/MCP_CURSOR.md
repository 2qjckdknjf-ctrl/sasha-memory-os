...
Tools include `memory.search`, `memory.get`, `memory.export`, `memory.embed`, `memory.embed_missing`, `extraction.preview`, `extraction.apply`, `capture.*`, `connections.*`, `oauth.*`, `consolidation.run`, `outbox.*`, `jobs.get`, `jobs.dead_letter_stale`.
